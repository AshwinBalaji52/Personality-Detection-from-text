...
class trainBuild:
	...
	def getValues(self):
		...
		self.data = pd.read_csv("single_essays.csv")
	...
	def getStatusProcessed(self):
		...
		self.data.to_csv("clean/train_single_essays.csv", index = False, header = False)
		mat = [] ## store processed numerical vectors
		for index, row in self.data.iterrows():
			mat.append(status[index] + row[2:].values.tolist())
		## write to file
		pd.DataFrame(mat).to_csv("clean/train_single_essay_v2.csv", index = False, header = False)
x = trainBuild()
x.getValues()
