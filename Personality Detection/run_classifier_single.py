...
def showOutput(train_labels, predict,test_data, method):
    ...
    results1 = map(int, list_output)
    results = map(bool, results1)
    # print(results)
    df1 = DataFrame({'Personality':label,'Result(True/False)':results,'Result(0/1)':results1})
    print(df1)
...
with open('clean/train_essay_v2.csv') as csv_file:
    ...
    with open('clean/train_single_essay_v2.csv') as csv_file:
